...
std::vector<Label> get_candidates_with_splitting(const std::vector<Word>& words, double X0, double Y0, double X1, double Y1, double baseline_y, int effective_mz_max =MZ_MAX) {
    ...
    for (const auto& w : words) {
        ...
        auto split_candidates = split_grouped_numbers(t, b,effective_mz_max);
...
std::pair<json, double> compute_from_vector_pdf_algo1(const std::string& pdf_path) {
    ...
    auto candidates = get_candidates_with_splitting(words, X0, Y0, X1, Y1, baseline_y, effective_mz_max);

    auto t0 = std::chrono::high_resolution_clock::now();
    ProfilerStart("myprofiler.out");
...
// process_folder
std::vector<json> process_folder(const std::string& folder_path, const std::string& output_json = "Algo1_Parallel_MPI_Cpp.json", int rank = 0, int size = 1) {
...
int main(int argc, char** argv) {
    ...
    auto t0 = std::chrono::high_resolution_clock::now();
    auto all_results = process_folder(folder, "Algo1_Parallel_MPI_Cpp.json", rank, size);
    auto t1 = std::chrono::high_resolution_clock::now();
